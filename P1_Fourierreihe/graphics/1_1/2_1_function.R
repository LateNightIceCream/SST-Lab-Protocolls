...
p <- ggplot(data.frame(x=c(0,2), y=c(0,2)), aes(x=x)) +
        ...
        scale_y_continuous(limits=c(0, 1.6180339887498948), breaks=c(0,A, 1.6180339887498948), labels=c(0, "A", "")) +
...
for (n in -1:1) {
  ...
  p <- p + geom_segment( x = -tau/2 + n*delta, y = 0, xend = -tau/2 + n*delta, yend = A, color=function_color, linetype="dashed")+
           geom_segment( x =  tau/2 + n*delta, y = 0, xend = tau/2 + n*delta,  yend = A, color=function_color, linetype="dashed")
}
